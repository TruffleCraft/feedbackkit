...
## ADR-009 · 2026-07-13 · P0-Validierungs-Spike übersprungen

Der Spike wurde auf Michels Entscheidung übersprungen. Die Kernthese-Validierung verschiebt sich auf die SCTT-Migration am P1-Exit (echte Feedbacks + Completion-Funnel-KPI = dieselbe Evidenz auf der echten Instanz). Eval-Fixtures (10 anonymisierte SCTT-Feedbacks) und das p90-<-8-s-Ziel sind in den P1-LLM-Baustein gewandert; der BugDrop-Fork-Check läuft als erste P1-Aufgabe. Bewusst akzeptierter Tradeoff: P1 baut auf einer erst in P1 validierten Prämisse — beide Review-Modelle hatten den Spike als zentralen De-Risking-Schritt markiert; Mitigation ist die scharfe Funnel-Beobachtung ab dem ersten SCTT-Tag. Verworfen: Spike als harter Gate vor P1 (Michel: SCTT als eigene Low-Risk-Property liefert die Daten ohnehin).
