...
- **LLM-Extract-then-Form:** 1 Call (multimodal, Screenshot ~800px), `need_fields` → `extracting` (nach 4 s wird „Jetzt senden" **primärer Button**; 15 s Hard-Timeout) → `completing_fields` (alle Pflichtfelder, Extrahiertes vorbefüllt+editierbar+markiert; „Trotzdem senden"-Textlink; Close = auto-Trotzdem via keepalive; Feld-Ceiling >3 → direkt accepted_incomplete); 2. POST deterministisch, Server re-validiert + re-derived Titel
...
- Hardening-Baseline: verankerte Origin-Regex (VOS-Bug nicht kopieren!), Honeypot, Length-Caps, serverseitige Type-Validierung, per-IP-Rate-Limit (atomarer D1-Upsert), Upload-Allowlist + Magic-Byte + Caps, benannte Fehlerklassen, CORS reflektiert (nie `*` auf APIs)
...
- **Webhook-Sink** (~1 PT: normalisierter JSON-POST pro Feedback → n8n/Zapier/Actions binden JEDES Tool an)
