...
You host it yourself: **one Cloudflare Worker** (+ D1 + R2, free tiers), one `pnpm deploy`, one snippet. See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) for the request/data flow. You bring your own LLM key — see [docs/MODELS.md](docs/MODELS.md) for a verified, cost-ranked model recommendation (default `gemma-3-27b-it` — open-weights, so the same model runs hosted or fully on-prem; managed and free options included).
